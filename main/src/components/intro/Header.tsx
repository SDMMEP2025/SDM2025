...
export function Header() {
  ...
  useMotionValueEvent(aboutPhase, 'change', (v) => {
    const onHome = restPath === '/' // '/' 또는 '/en' 루트
    if (!onHome) return
    setAboutActiveOnHome(v >= 1.0)
    console.log('hello', aboutActiveOnHome)
  })

  useEffect(() => {
    if (restPath !== '/') setAboutActiveOnHome(false)
  }, [restPath])
